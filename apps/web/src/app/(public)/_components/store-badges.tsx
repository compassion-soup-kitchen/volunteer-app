 ...
 *
 * The store links are placeholders until the app is published. While a URL is a
 * placeholder the badge renders as a non-navigating <button> labelled "coming
 * soon" — a real <a href="#"> would scroll the page to the top and mislead
 * screen-reader / keyboard users into thinking they were leaving for a listing.
 * Swap the constants for real listing URLs and the badges become links.
 */
...
function Badge({
  ...
  return (
    <a href={href} aria-label={label} className={cn(badgeBase, className)}>
      {children}
    </a>
  );
}
...
export function AppStoreBadge({ className }: { className?: string }) {
  return (
    <Badge href={APP_STORE_URL} label="Download on the App Store" className={className}>
...
export function GooglePlayBadge({ className }: { className?: string }) {
  return (
    <Badge href={PLAY_STORE_URL} label="Get it on Google Play" className={className}>
